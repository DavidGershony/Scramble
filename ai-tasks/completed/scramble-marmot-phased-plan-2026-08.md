...
| **P9** | **Hardening** | M | Session-open hydration, ~~quarantine~~ (**dropped 2026-09-14** — it was upstream's per-group *hydration* quarantine, carried into this row by name from `survives-rewrite-diff-2026-07.md:169` while that same document's line 321 had already ruled it out for v1: "v1 can drop forensics, quarantine hardening, and deferred-peel caps". Upstream needs a blast-radius container because its hydration is eager and account-wide; ours is lazy and per-group, so a failed open isolates itself. Replaced by a smaller ask: make `OpenAsync`'s failure legible), stranded-pending-commit crash recovery, deferred-peel retry lifecycle with the flood cap (256) — **the retry budget (32) is dropped, 2026-09-13: a held message is skipped at an epoch that already refused it, and the delivery window retires it a few epochs on, so attempts per message are bounded at roughly six by construction and a count has nothing left to do** — ~~queued-intent drain polish~~ (**there is no drain to polish** — 2026-09-15; the outbound send path was never built, see `remaining-work-2026-09.md` §8), snapshot-fallback peel. | Crash-recovery tests (kill between stage and confirm, between confirm and merge) leave a consistent group; epoch-boundary messages survive via snapshot-fallback peel. | Unit + `DarkMatterInterop` |
| **P10** | **Capabilities / feature registry** (exit criterion **done** 2026-09-15; the upgrade flow is *inert*, not dead — `remaining-work-2026-09.md` §2) | S–M | Minimal static registry is a **P4/P6 prerequisite** (create/invite/join gates); this phase completes profile negotiation, the upgrade flow, and `legacy_compatibility_profile` handling if §5 Q2 says we need it. | Capability-mismatch rejections match mdk's error taxonomy. | Unit + `ConformanceVector` |
